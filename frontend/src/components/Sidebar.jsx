  ...
  return (
    <aside className="creator-sidebar">
      <div className="creator-sidebar__brand">
        <img className="creator-sidebar__brand-image" src="/images/Brand%20Logo.png" alt="Shades of SG Creator" />
      </div>
      <div className="creator-sidebar__section-label">Creator Portal</div>
      <nav aria-label="Creator navigation" className="creator-sidebar__nav">
        {creatorLinks.map((item) => (
          <NavLink className={({ isActive }) => `creator-sidebar__link ${isActive ? 'active' : ''}`} key={item.to} to={item.to}>
            <span className="creator-sidebar__icon" aria-hidden="true">
              <SidebarIcon type={item.icon} />
            </span>
            <span className="creator-sidebar__label">{item.label}</span>
            {item.badge && <span className="creator-sidebar__badge">{item.badge}</span>}
          </NavLink>
        ))}
      </nav>
      <div className="creator-sidebar__art" aria-hidden="true" />
